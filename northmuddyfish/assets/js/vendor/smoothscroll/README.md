...
```js
import( './vendor/smoothscroll/smoothscroll.min.js' )
    .then((smoothscroll) => {
        smoothscroll.polyfill();
    })
    .catch(error => {
        console.log( 'smoothscroll failed to load.' );
    });
```
...
CSS that can be used with it.
```css
html{
	@media screen and (min-width: 1023px) {	// Desktop only
		--scroll-behavior: smooth;			// Native Smooth Scroll Polyfill smoothscroll.js
		scroll-behavior: smooth;			// Native Smooth Scroll Polyfill smoothscroll.js
	}
}
```
